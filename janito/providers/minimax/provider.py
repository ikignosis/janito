...
from janito.providers.minimax.model_info import MODEL_SPECS, DEFAULT_MODEL
...
class MiniMaxProvider(LLMProvider):
    ...
    def __init__(
        ...
        # Initialize API key and configure MiniMax-specific settings
        if self.available:
            self._initialize_minimax_config()

    def _initialize_minimax_config(self):
        """Initialize MiniMax-specific configuration."""
        # Initialize API key
        api_key = self.auth_manager.get_credentials(self.name)
        if not api_key:
            from janito.llm.auth_utils import handle_missing_api_key

            handle_missing_api_key(self.name, "MINIMAX_API_KEY")

        # Set API key in config
        if not self.config.api_key:
            self.config.api_key = api_key

        # Set the MiniMax OpenAI-compatible API endpoint
        self.config.base_url = "https://api.minimax.io/v1"

    def create_driver(self) -> OpenAIModelDriver:
        """
        Create and return a new OpenAIModelDriver instance for MiniMax.
...
LLMProviderRegistry.register(MiniMaxProvider.NAME, MiniMaxProvider)
