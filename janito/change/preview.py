...
def setup_preview_directory() -> Path:
    ...
    # Copy existing files to preview directory if workdir exists
    if config.workdir.exists():
        shutil.copytree(config.workdir, preview_dir, dirs_exist_ok=True, 
                       ignore=shutil.ignore_patterns('.janito', '.git'))
        
    return preview_dir

def setup_workdir_preview() -> tuple[Path, Path]:
