...
For direct execution, use: python -m janito.tools.files.read_file_lines [args]
...
class ReadFileLines(BaseTool):
    ...
    def run(
        ...
        try:
            ...
            actual_from = (
                from_line - 1 if from_line is not None else 0
            )  # Convert to 0-based
            actual_to = (
                to_line if to_line is not None else total_lines
            )  # Keep as 1-based for slicing
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Read specific line ranges from a file (1-based indexing)"
    )
    parser.add_argument("filepath", help="File path to read")
    parser.add_argument(
        "--from-line", "-f", type=int, help="Starting line number (1-based)"
    )
    parser.add_argument(
        "--to-line", "-t", type=int, help="Ending line number (1-based)"
    )
    parser.add_argument(
