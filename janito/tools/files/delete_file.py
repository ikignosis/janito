...
For direct execution, use: python -m janito.tools.files.delete_file [args]
    ...
    def run(self, filepath: str, force: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            if e.errno == 39:  # Directory not empty
                ...
                self.report_error(f"OS Error deleting file: {e!s}")
                return {
                    "success": False,
                    "error": str(e),
                    "filepath": filepath,
...
def main():
    ...
    result = tool_instance.run(filepath=args.filepath, force=args.force)
