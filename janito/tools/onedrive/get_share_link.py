...
class GetOneDriveShareLink(BaseTool):
    ...
    def run(
        self, path: str, link_type: str = "view", scope: str = "anonymous"
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            request_body = {"type": link_type, "scope": scope}

            self.report_progress(f" Creating {link_type} link with {scope} scope...")

            response = client._make_request(
                method="POST", endpoint=endpoint, json_data=request_body, timeout=30
            )
