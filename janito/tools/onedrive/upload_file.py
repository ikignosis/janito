...
class UploadOneDriveFile(BaseTool):
    ...
    def run(
        ...
        try:
            ...
            if file_size > self.LARGE_FILE_THRESHOLD:
                result = self._upload_large_file(
                    client, endpoint, file_bytes, overwrite
                )
            else:
            ...
            self.report_error(f"Failed to upload file: {e!s}")
            return {
                "success": False,
    ...
    def _upload_simple(
        ...
        if response.status_code >= 400:
            ...
            return {"success": False, "error": f"Upload failed: {error_message}"}
    ...
    def _upload_large_file(
        ...
        while offset < total_size:
            ...
            self.report_progress(
                f" Uploading chunk {chunk_num} ({len(chunk)} bytes)..."
            )
