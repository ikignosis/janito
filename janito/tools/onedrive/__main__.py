...
Run with: python -m janito.tools.onedrive [command] [options]

Examples:
    python -m janito.tools.onedrive list-files
    python -m janito.tools.onedrive list-files --path Documents
    python -m janito.tools.onedrive search-files --query report
    python -m janito.tools.onedrive read-file --path "Documents/readme.txt"
...
from .device_code_auth import DeviceCodeAuth, clear_tokens, store_token_data
...
def authenticate_command(args) -> int:
    ...
    if not client_id:
        ...
        print("3. Enter name: 'janito OneDrive'", file=sys.stderr)
        print(
            "4. For personal accounts: Select 'Accounts in any personal Microsoft account'",
            file=sys.stderr,
        )
        print(
            "5. For work accounts: Select 'Accounts in any organizational directory'",
            file=sys.stderr,
        )
...
Required secret:
  janito --set-secret azure_client_id=your-client-id
    ...
    list_parser.add_argument(
        "--path", "-p", default="", help="Folder path to list (default: root)"
    )
    list_parser.add_argument(
        "--limit",
        "-l",
    ...
    list_parser.add_argument(
        "--folders-only", "-f", action="store_true", help="List only folders"
    )
    list_parser.add_argument(
    ...
    search_parser.add_argument("--folder", "-f", help="Limit search to specific folder")
    search_parser.add_argument(
        "--type", "-t", help="Filter by file extension (e.g., docx, pdf)"
    )
    search_parser.add_argument(
        "--limit", "-l", type=int, default=20, help="Maximum results (default: 20)"
    )
    search_parser.add_argument(
        "--json", "-j", action="store_true", help="Output in JSON format"
    )
