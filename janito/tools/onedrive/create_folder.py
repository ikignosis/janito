...
class CreateOneDriveFolder(BaseTool):
    ...
    def run(self, path: str, conflict_behavior: str = "fail") -> dict[str, Any]:
        """
        ...
        try:
            ...
            self.report_progress(
                f" Creating folder '{folder_name}' in {parent_path or '/'}"
            )

            # Map conflict behavior to Graph API value
            conflict_map = {"fail": "fail", "replace": "replace", "rename": "rename"}
            ...
            folder_info = {
                ...
                "child_count": response.get("folder", {}).get("childCount", 0)
            ...
            error_msg = str(e)
            if (
                "nameAlreadyExists" in error_msg
