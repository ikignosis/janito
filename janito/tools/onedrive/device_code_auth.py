...
class DeviceCodeAuth:
    ...
    def get_device_code(self) -> tuple[str, str]:
        ...
        verification_url = result.get("verification_url") or result.get(
            "verification_uri"
        )
    ...
    def poll_for_token(self, print_status: bool = True) -> dict:
        ...
        while time.time() < self._poll_expires_at:
            ...
            if error == "authorization_pending":
                ...
                raise Exception(
                    "Authentication declined by user. Please run the auth command again."
                )

            elif error == "bad_verification_code":
                raise Exception(
                    "Invalid device code. Please run the auth command again."
                )

            elif error == "expired_token":
                raise Exception(
                    "Device code expired. Please run the auth command again."
                )
    ...
    def authenticate(client_id: str) -> dict:
        ...
        print(
            "  4. Click 'Continue' to grant permissions", file=__import__("sys").stderr
        )
        print("\n  Waiting for authentication...\n", file=__import__("sys").stderr)
