                        ...
                        if (
                            ...
                            if is_tool(attr):
                                ...
                                def make_class_tool(cls):
                                    ...
                                    class_tool_wrapper._tool_permissions = getattr(
                                        cls, "_tool_permissions", ""
                                    )
                                    # Propagate the load validation hook for later introspection
                                    class_tool_wrapper.should_load = getattr(
                                        cls, "should_load", None
                                    )

                                    # Preserve type hints (excluding 'self')
                                    class_tool_wrapper.__annotations__ = {
                                        k: v
                                        for k, v in run_type_hints.items()
                                        if k != "self"
                                    }
