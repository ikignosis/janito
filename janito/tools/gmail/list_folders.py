...
class ListFolders(BaseTool):
        ...
        try:
            ...
            if include_counts and folders:
                self.report_progress(
                    f" Getting email counts for {len(folders)} folders..."
                )
            ...
            self.report_error(f"Error listing folders: {e!s}")
            return {"success": False, "error": f"Failed to list folders: {e!s}"}

    def _parse_folder_entry(self, folder_data: bytes) -> dict[str, Any] | None:
        """
    ...
    def _add_folder_counts(
        self, mail: imaplib.IMAP4_SSL, folders: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
