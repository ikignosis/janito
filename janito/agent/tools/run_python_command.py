...
class RunPythonCommandTool(ToolBase):
    ...
    def call(self, code: str, timeout: int = 60, require_confirmation: bool = False, interactive: bool = False) -> str:
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w+', suffix='.py', prefix='run_python_', delete=False, encoding='utf-8') as code_file, \
                 tempfile.NamedTemporaryFile(mode='w+', prefix='run_python_stdout_', delete=False, encoding='utf-8') as stdout_file, \
                 tempfile.NamedTemporaryFile(mode='w+', prefix='run_python_stderr_', delete=False, encoding='utf-8') as stderr_file:
